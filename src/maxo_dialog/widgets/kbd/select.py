...
from maxo_dialog.widgets.common import ManagedWidget, WhenCondition
from maxo_dialog.widgets.common.items import (
    ItemsGetterVariant,
    get_items_getter,
)
from maxo_dialog.widgets.text import Case, Text
...
class Multiselect(StatefulSelect[T], Generic[T]):
    ...
    async def _on_click(
        ...
        await self.set_checked(
            callback,
            item_id,
            not self.is_checked(item_id, manager),
            manager,
        )
